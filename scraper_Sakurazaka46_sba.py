...
def get_profile(member_id: str):
    result = {}
    profile_url = f"https://archive.sakamichi.co/sakura/members/{member_id}/"
...
def get_blog_url_list(member_id: str, previous_blog_url_list: list):
    current_page = 1
    current_url = f"https://archive.sakamichi.co/sakura/members/{member_id}"
    articles_url_list = []
    ...
    while True:
        ...
        for a in a_list:
            ...
            url_keyaki=url_with_param.replace("https://sakurazaka46.com/s/s46/diary/detail/","https://www.keyakizaka46.com/s/k46o/diary/detail/")
            if url_keyaki in previous_blog_url_list:
                return articles_url_list
        ...
        if next_a_div:
            current_page += 1
            current_url = f"https://archive.sakamichi.co/sakura/members/{member_id}/p{current_page}"
...
def get_blog_content(url: str, repo_name: str):
    while True:
        try:
            ...
            sba_url = url.replace(
                "https://sakurazaka46.com/s/s46/diary/detail/",
                "https://archive.sakamichi.co/sakura/blogs/",
            )
            soup = BeautifulSoup(requests.get(sba_url).content, "lxml")

            # https://archive.sakamichi.co/sakura/blogs/35793
            data["title"] = (
