...
def get_profile(member_id: str):
    result = {}
    profile_url = f"https://archive.sakamichi.co/hinata/members/{member_id}/"
...
def get_blog_url_list(member_id: str, previous_blog_url_list: list):
    current_page = 1
    current_url = f"https://archive.sakamichi.co/hinata/members/{member_id}"
    articles_url_list = []
...
def get_blog_content(url: str, repo_name: str):
    while True:
        try:
            ...
            sba_url = url.replace(
                "https://www.hinatazaka46.com/s/official/diary/detail/",
                "https://archive.sakamichi.co/hinata/blogs/",
            )
            soup = BeautifulSoup(requests.get(sba_url).content, "lxml")

            # https://archive.sakamichi.co/hinata/blogs/35793
            data["title"] = (
